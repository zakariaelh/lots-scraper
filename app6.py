...
def generate_table(df_j, type_):
    ...
    if type_ == 'house':
        ...
        html.Tbody([html.Tr(
            ...
            [html.Td(html.A('check listing', href = df_.iloc[i]['url'], target = '_blank'))]
                            ) 
...
app.layout = html.Div([
	...
	html.Div(
        ...
        html.Table(id = 'table_lots', className = 'scrolltable')],
        style = {'overflow-x' : 'auto'}
        ),
	html.H3('Houses'),
	#html.Table(id = 'table_houses'),
	html.Div(
        [html.Thead(html.Tr([html.Th(html.Button(col, id = col)) for col in cols_h]), className = 'scrolltable',
		id = 'thead_cols_h',
        	),
        html.Table(id = 'table_houses', className = 'scrolltable')],
        style = {'overflow-x' : 'auto'}
        ),
	...
	html.Div(
    [html.Thead(html.Tr([html.Th(html.Button(col, id = col + '__')) for col in cols_f]), className = 'scrolltable',
		id = 'thead_cols_f',
    	),
    html.Table(id = 'table_filtered', className = 'scrolltable')],
    style = {'overflow-x' : 'auto'}
    ),
    html.Div(
    [
...
def filter_data(d_houses, d_lots, b, 
	...
	if b is not None:
		...
		d_lots.to_csv(DOWNLOAD_DIRECTORY + 'd_lots_filtered.csv')
		
		try:
		    l_k = [k_a, k_ar, k_l, k_ln, k_n, k_p, k_av, k_m, k_u] #n_clicks
		    l_k_t = [k_a_t, k_ar_t, k_l_t, k_ln_t, k_n_t, k_p_t, k_av_t, k_m_t, k_u_t] #timestamps
		    print('l_k ', l_k)
		    print('l_k_t', l_k_t)
		    if all(v is None for v in l_k_t):
		    	print(d_lots)
